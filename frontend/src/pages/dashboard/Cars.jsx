...
export default function Cars() {
  ...
  const limit = 20;
  const canAdd = user?.role?.name === 'superadmin' || user?.permissions?.cars_add;
  ...
  return (
    <div dir="rtl">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <button
          onClick={fetchCars}
          className="p-2 border border-border-muted rounded-sm hover:bg-background text-text-secondary"
          title="تحديث"
        >
          <RefreshCw size={16} />
        </button>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading && (
                <tr>
                  <td colSpan={8} className="text-center py-12 text-text-muted">جاري التحميل…</td>
                </tr>
              )}
              {!loading && cars.length === 0 && (
                <tr>
                  <td colSpan={8} className="text-center py-12 text-text-muted">لا توجد عربيات</td>
                </tr>
              )}
              {!loading && cars.map((car, idx) => {
                ...
                return (
                  <tr
                    key={car.id}
                    className={`border-b border-border-muted last:border-0 hover:bg-background/50 transition-colors cursor-pointer ${idx % 2 === 0 ? '' : 'bg-background/30'}`}
                  ...
                  >
                    <td className="py-3 px-4">
                      {thumb ? (
                        ...
                        <div className="w-12 h-9 rounded-sm bg-background flex items-center justify-center text-text-muted text-xs">—</div>
                      )}
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4 font-semibold text-text-primary">{formatEGP(car.listingPrice)}</td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4">
                      <button
                        ...
                        className="p-1.5 text-text-muted hover:text-primary rounded-sm hover:bg-primary-light"
                      >
                        <Eye size={16} />
                      </button>
                    </td>
